...
const agent = {
  ...
  source: {
    ...
    syncedAt: '2026-08-30T18:00:00.000Z',
  },
}
...
const status = {
  id: 'git',
  status: 'ok',
  lastSyncAt: '2026-08-30T18:00:00.000Z',
...
describe('AgentsWorkspace', () => {
  ...
  it('syncs only Friday-owned registry state from Git and refreshes status', async () => {
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith('/api/agents/registry/sync', expect.objectContaining({
      method: 'POST',
      body: JSON.stringify({}),
    })))
  ...
  it('manually asks the selected agent and renders local-only provenance and execution=false', async () => {
    ...
    expect(screen.getByText(/No actions executed/i)).toBeInTheDocument()
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith('/api/agents/proxmox-observer/ask', expect.objectContaining({
      method: 'POST',
      body: JSON.stringify({ prompt: 'Check VM 100' }),
    })))
  })
