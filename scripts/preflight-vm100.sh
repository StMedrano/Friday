...
printf '%s\n' 'Friday VM100 preflight (read-only)'
printf '%s\n' '----------------------------------'
check_cmd git
check_cmd docker
check_cmd curl
...
if [ -S /var/run/docker.sock ]; then
  echo 'ok   docker socket      present (live override can mount read-only)'
else
  echo 'WARN docker socket      /var/run/docker.sock not present'
fi
...
if [ -f .env ]; then
  ...
  if [ "$MODE" = 'live' ] && [ "$DOCKER_ENABLED" = 'true' ] && [ ! -S /var/run/docker.sock ]; then
    echo 'FAIL live Docker requested but Docker socket is unavailable' >&2
    FAILED=1
  fi
...
if [ "$FAILED" -ne 0 ]; then
  echo 'Friday preflight failed.' >&2
  exit 1
fi

echo 'Friday preflight completed without blocking errors.'
