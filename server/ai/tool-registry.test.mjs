...
test('audit receives completed and blocked outcomes', async () => {
  ...
  await executeAgentTool({ registry, agent: agent(), request: { tool: 'health.check' }, audit: async event => events.push(event) })
  await executeAgentTool({ registry, agent: agent(), request: { tool: 'unknown.tool' }, audit: async event => events.push(event) })
