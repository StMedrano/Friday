...
**Architecture:** NPM on VM100 (`192.168.1.124`) owns private TCP/80, TCP/81, and TCP/443 and proxies the FRIDAY FQDN to VM102 (`192.168.1.64:3010`). Cloudflare is used only for DNS-01 certificate validation. VM102 keeps UFW as the host policy and adds a root-only `DOCKER-USER` guard so Docker-published TCP/3010 cannot bypass the source restriction.

**Tech Stack:** Docker Engine/Compose, Nginx Proxy Manager, AdGuard Home, Cloudflare DNS API + Let's Encrypt DNS-01, Twingate, Ubuntu 26.04 UFW, iptables-nft-compatible `DOCKER-USER`, systemd, Node 22 tests, GitHub Actions, Omada SDN.
...
- UFW alone is not accepted for the Docker-published-port boundary; `DOCKER-USER` must exist and enforce the source restriction.
...
- Do not alter NPM database, certificates, or unrelated proxy hosts except the explicit FRIDAY certificate/proxy host.
...
- Create `ops/friday-backend-guard.service` — reapplies the guard after Docker starts/restarts and has no automatic rule-removal action.
- Create `ops/friday-backend-guard.test.mjs` — fake-iptables tests requiring no root or real firewall.
...
- AdGuard, Omada DHCP/NAT, Cloudflare, and Twingate configuration
...
- Produces: systemd unit restarted with Docker and CI that fails if runtime/browser code references privileged host-routing operations.
...
`PartOf=docker.service` propagates Docker restarts to this unit; `After=docker.service` makes the oneshot reapply after Docker recreates its filter chains. There is intentionally no `ExecStop`.
...
Add immediately after that CI step:
...
- Omada must have no WAN port-forward/DMZ/1:1-NAT path exposing VM100 80/81/443 or VM102 3010.
...
```text
Adds only host-operator routing guard assets, CI isolation checks, the approved design/plan, and runbook. It does not add authentication or infrastructure mutation authority to the FRIDAY web/server runtime. Live NPM, AdGuard, UFW, Cloudflare, Omada, and Twingate changes remain separately approval-gated.
```
...
### Task 6: Prove no WAN exposure, then establish DNS-01 and the FRIDAY proxy host

**Files:** live Omada, AdGuard, Cloudflare, and NPM only.

**Interfaces:**
- Consumes: NPM on host 443 and still-open VM102 backend.
- Produces: trusted HTTPS through NPM with no WAN NAT path to FRIDAY/NPM-admin.

- [ ] **Step 1: Audit Omada WAN NAT before creating the private hostname path**

In Omada Controller inspect all enabled rules:

```text
Omada v6: Network Config > Transmission > NAT > Port Forwarding
Omada v5: Settings > Transmission > NAT > Port Forwarding
Also inspect: One-to-One NAT, Disable NAT, and any DMZ destination setting.
```

Expected: no enabled WAN rule exposes any of these destinations:

```text
192.168.1.124 TCP 80
192.168.1.124 TCP 81
192.168.1.124 TCP 443
192.168.1.64  TCP 3010
```

If an existing rule/DMZ/1:1 mapping exposes one of those paths, STOP. Do not delete it blindly; identify what depends on it and request explicit approval for that separate removal.

- [ ] **Step 2: Verify/create AdGuard rewrite and upstream DNS**
...
Expected: first returns exactly `192.168.1.124`; second returns public address(es).

- [ ] **Step 3: Verify public DNS is not exposing the private FRIDAY destination**

```bash
dig @1.1.1.1 friday.mytechtactics.com +short
```

Expected: no `192.168.1.124`. If a public FRIDAY A/AAAA/CNAME exists, stop and review it.

- [ ] **Step 4: Create a dedicated Cloudflare token in the Cloudflare UI**
...
Do not put the token in a shell or repo. Copy it directly from Cloudflare into NPM's DNS-provider credential editor.

- [ ] **Step 5: Request certificate in NPM**
...
Provider: Cloudflare
In the provider credential editor, enter the newly-created token after: dns_cloudflare_api_token =
```
...
- [ ] **Step 7: Validate HTTPS and redirect before firewalling**
...
- [ ] **Step 3: Record UFW pre-state and inspect listeners**
...
sudo ss -lntH
sudo cat /var/lib/friday-backend-guard/ufw-before-state
```

If an unexpected externally-bound VM102 service exists beyond SSH/FRIDAY, stop before default-deny.

- [ ] **Step 4: Establish UFW policy while preserving SSH**
...
- [ ] **Step 7: Validate denied direct path and working HTTPS from a non-VM100 LAN client**
...
Expected: direct 3010 fails; HTTPS succeeds.
...
if [ "$(sudo cat /var/lib/friday-backend-guard/ufw-before-state)" = inactive ]; then
  sudo ufw disable
fi
```

Stop and diagnose; do not weaken NPM or AdGuard.
...
- [ ] **Step 2: Create Twingate groups if they do not already exist**
...
Add the Owner account to both groups. Future non-admin FRIDAY users belong only to `FRIDAY Access`.

- [ ] **Step 3: Create FRIDAY FQDN Resource**

```text
Name: FRIDAY Control Plane
Address: friday.mytechtactics.com
Port: TCP 443 only
Access group: FRIDAY Access
```

- [ ] **Step 4: Create separate NPM admin Resource**

```text
Name: NPM Admin
Address: 192.168.1.124
Port: TCP 81 only
Access group: FRIDAY Admins
```

Do not create a Resource for `192.168.1.64:3010`.

- [ ] **Step 5: Validate from a remote Twingate client**
...
Expected: HTTPS succeeds with a trusted certificate; direct 3010 fails.

- [ ] **Step 6: Validate NPM admin separately**

From an identity in `FRIDAY Admins`, open:

```text
http://192.168.1.124:81
```

Expected: NPM loads. An identity only in `FRIDAY Access` must not receive the NPM Admin Resource.
...
- [ ] **Step 3: Record the acceptance matrix**
...
Omada WAN -> VM100 80/81/443                         NO FORWARD/DMZ/1:1 NAT
Omada WAN -> VM102 3010                              NO FORWARD/DMZ/1:1 NAT
```
...
- Omada has no WAN forwarding/DMZ/1:1-NAT exposure for VM100 80/81/443 or VM102 3010.
...
Remove or replace stale statements claiming NPM is offline or direct 3010 is the canonical user path.
...
Present exact PR head SHA, exact-head CI, review result, NPM 80/81/443 evidence, Omada no-WAN-exposure evidence, AdGuard DNS evidence, LAN/Twingate HTTPS evidence, VM100-to-3010 allow evidence, and ordinary-client 3010 deny evidence.
