...
Proxmox host
  ...
  -> CT108 friday-ollama (10.1.10.12:11434, nic1 / VLAN 10)
  -> qwen3:4b-instruct
  -> VM102 Friday (10.1.10.11, nic1 / VLAN 10)
```

The nic1 path above is verified live: CT108's running-container interface reports `10.1.10.12/24`, and VM102 reaches TCP/11434, `/api/tags`, and `/api/chat` from `10.1.10.11` over `eth1`. The CT108 firewall should allow TCP/11434 only from VM102. The model is expected to report `100% GPU` in `ollama ps` during inference. Legacy vmbr0 addresses remain rollback references only.
